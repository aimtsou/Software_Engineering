...
/*******************************************************************************/
		...
		" (an error code has normally been"+
            ...
            // Terminate call
            // If request is BYE and update totalCost
            if (request.getMethod().equals(Request.BYE)) {
            	...
            	// Calculate the charge of the call
            	Billing dm = new Billing();
            	totalCost = (long) (dm.findAccountType(userCharged) * diff /1000);
            	// Update the Billing.xml file
            	File xmlfile = new File ("./gov/nist/sip/proxy/BillingCosts.xml");
            	dm.updateAccount(xmlfile, userCharged, totalCost);
...
/*****************************************************************************/            
		...
		clonedRequest.addHeader(via);
		  ...
		  return;
            ...
            Request clonedRequest = (Request) request.clone(); 
     		...
     		// Get the full Ids of the Caller and the Callee
     		String callerID = from.getAddress().getURI().toString();
     		String calleeID = to.getAddress().getURI().toString();
     		
     		// Get their Usernames
     		String callerUsername = callerID.substring(callerID.indexOf("sip") + 4, callerID.indexOf("@"));
     		String calleeUsername = calleeID.substring(calleeID.indexOf("sip") + 4, calleeID.indexOf("@"));
     		
     		Blocking blocking = new Blocking();
     		
     		// If the Callee has blocked the Caller
     		if (blocking.checkForBlocks(callerUsername, calleeUsername)) {
     			
     			//  Return a busy tone for blocked users
    ...
    public void processResponse(ResponseEvent responseEvent) {
	    ...
	    // Check if callee answered. If they answered return code of invite is 200 
        // Start charging
	     } else if ((cseqHeader.getMethod().equals("INVITE")&&(response.getStatusCode() == 200))) {
	    	
	    	Response clonedResponse = (Response) response.clone();
            FromHeader from = (FromHeader) clonedResponse.getHeader(FromHeader.NAME);
	     	String callerID = from.getAddress().getURI().toString();
	     	
	     	// Initialize Caller
	     	String callerUsername = callerID.substring(callerID.indexOf("sip") + 4, callerID.indexOf("@"));
	     	userCharged = callerUsername;
